...
var playercol = player.GetComponent(playercol);
	...
	if(hp <= 0){
		...
		playercol.score = playercol.score + 100;
			if(randomNumber == 2.0){
				Instantiate(Resources.Load('Coeur plein') as GameObject, coeur ,Quaternion.identity);
			}
			else if(randomNumber == 1.0){
				Instantiate(Resources.Load('Pièce Mercure') as GameObject, mercure, Quaternion.identity);
			}
		}
	}
}	
...
if (PlayerPrefs.GetInt("DeadDroite") == 1){
...
function Damage(DamageCount : float){
	...
	var gauche : Transform = transform.Find("Gaucheennemi");
		...
		var morthaut : GameObject = Instantiate(Resources.Load('Mortennemihaut') as GameObject, transform.position , transform.rotation);
				Destroy(morthaut, 1);
		Destroy(gameObject);					
		}
		if (droite.renderer.enabled == true){
					...
					droite.renderer.enabled = false;
					bas.renderer.enabled = false;
		var mortdroite : GameObject = Instantiate(Resources.Load('Mortennemidroite') as GameObject, transform.position , transform.rotation);
		...
		if (gauche.renderer.enabled == true){
					...
					droite.renderer.enabled = false;
					bas.renderer.enabled = false;
		var mortgauche : GameObject = Instantiate(Resources.Load('Mortennemigauche') as GameObject, transform.position , transform.rotation);
		...
		if (bas.renderer.enabled == true){
					...
					droite.renderer.enabled = false;
					bas.renderer.enabled = false;
		var mortbas : GameObject = Instantiate(Resources.Load('Mortennemibas') as GameObject, transform.position , transform.rotation);
				Destroy(mortbas, 1);
		Destroy(gameObject);
		}
	}
...
scriptMove.canMove = false;
var hautrouge : GameObject = Instantiate(Resources.Load('Ennemihautrouge') as GameObject, this.transform.position ,this.transform.rotation);
...
scriptMove.canMove = false;
var gaucherouge : GameObject = Instantiate(Resources.Load('Ennemigaucherouge') as GameObject, this.transform.position ,this.transform.rotation);
	yield WaitForSeconds(0.2);
	Destroy(gaucherouge);
...
if (droite  == true){
		scriptMove.canMove = false;
		var droiterouge : GameObject = Instantiate(Resources.Load('Ennemidroiterouge') as GameObject, this.transform.position ,this.transform.rotation);
...
var basrouge : GameObject = Instantiate(Resources.Load('Ennemibasrouge') as GameObject, this.transform.position ,this.transform.rotation);
	yield WaitForSeconds(0.2);
	scriptMove.bas.renderer.enabled = true;
Destroy(basrouge);
scriptMove.canMove = true;
